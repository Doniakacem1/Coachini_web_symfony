...
class EventController extends AbstractController
{
    ...
    /**
     * @Route("/listallevents", name="listallevents")
     */
    public function mobileindex(NormalizerInterface $normalizer)
    {
        $repository=$this->getDoctrine()->getRepository(Event::class);
        $Events =$repository->findAll();
        $json=$normalizer->normalize($Events,'json',['groups'=>'events']);
    ...
    public function mobileAddEvent(Request $request,NormalizerInterface $normalizer)
    {
        ...
        $Event->setDateDebut(\DateTime::createFromFormat('Y-m-d', $request->get('dateDebut')));
        $Event->setHeureDebut($request->get('heureDebut'));
        $Event->setDateFin(\DateTime::createFromFormat('Y-m-d', $request->get('dateFin')));
        $Event->setHeureFin($request->get('heureFin'));
        $Event->setParticipation($request->get('participation'));
        $Event->setNbParticipant($request->get('nbParticipant'));
        $Event->setDescription($request->get('description'));
        $Event->setDbMap($request->get('dbMap'));
        $Event->setIdCoach($request->get('idCoach'));
        $Event->setMap($request->get('map'));
        $Event->setIdCatmobile($request->get('idCat'));
        $em ->merge($Event);
        $em->flush();
        $json=$normalizer->normalize($Event,'json',['groups'=>'events']);
        return  new Response('Event added succefully');
    }

    /**
     * @Route("/addpicture", name="addpicture")
     */
    public function mobileAddPicture(Request $request,NormalizerInterface $normalizer)
    {
        $em=$this->getDoctrine()->getManager();
        $Event=new Event();
        $Event->setImageFile($request->get('picture'));
        $em ->persist($Event);
        $em->flush();
        $json=$normalizer->normalize($Event,'json',['groups'=>'events']);
        return  new Response('Picture added succefully');
    }
     ...
     */
    public function mailing(Event $event,MailerInterface $mailer,Request $request,PaginatorInterface $paginator): Response
    {
    ...
    public function index(Request $request,PaginatorInterface $paginator): Response
    {
        ...
        $events = $this->getDoctrine()
            ->getRepository(Event::class)
            ->findByExampleField($compte->getId());
